...
int read_LCD_buttons() {
  //The Select button is the only one that matters for this game besides the code so i spent forever getting the joystick to work for nothing. :( it b like dat sometimes :/
  ...
  if(digitalRead(11) == HIGH ) return btnSELECT;
  if(ypos > 600) return btnUP;//??
  if(ypos < 400) return btnDOWN;//??
  if(xpos >600) return btnLEFT;//??
  if(xpos < 400) return btnRIGHT;
  return btnNONE;
  /*
